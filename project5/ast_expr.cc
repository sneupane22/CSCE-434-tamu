...
{
        Location* Right_Emit = right->Emit(codegen);
        Location* Left_Emit;
        if (left == NULL) {
                if (strcmp(op->str(), "-") == 0) 
                        return codegen->GenBinaryOp("-",
                                                    codegen->GenLoadConstant(0),
                                                    Right_Emit); 
                return NULL;
        }
        Left_Emit = left->Emit(codegen);
        return codegen->GenBinaryOp(op->str(),
                                    Left_Emit,
                                    Right_Emit);
}
...
{
        Location* Right_Emit = right->Emit(codegen);
        Location* Left_Emit  = left->Emit(codegen);
        if (strcmp(op->str(), ">") == 0)
                return codegen->GenBinaryOp("<", Right_Emit, Left_Emit);
        if (strcmp(op->str(), "<") == 0)
                return codegen->GenBinaryOp("<", Left_Emit, Right_Emit);
        if (strcmp(op->str(), ">=") == 0) {
                Location* tmp1 = codegen->GenBinaryOp("<", Right_Emit, Left_Emit);
                Location* tmp2 = codegen->GenBinaryOp("==", Left_Emit, Right_Emit);
                return codegen->GenBinaryOp("||", tmp1, tmp2);
        }
        if (strcmp(op->str(), "<=") == 0) {
                Location* tmp1 = codegen->GenBinaryOp("<", Left_Emit, Right_Emit);
                Location* tmp2 = codegen->GenBinaryOp("==", Left_Emit, Right_Emit);
                return codegen->GenBinaryOp("||", tmp1, tmp2);
        }
        return NULL;        
...
Location* EqualityExpr::Emit(CodeGenerator* codegen)
{
        ...
        Location* Left_Emit  = left->Emit(codegen);
        if (strcmp(op->str(), "==") == 0) {
                if (right->GetType() == Type::stringType)
                        return codegen->GenBuiltInCall(StringEqual,
                                                       Left_Emit,
                                                       Right_Emit);
                return codegen->GenBinaryOp("==", Left_Emit, Right_Emit);
        }
        if (strcmp(op->str(), "!=") == 0) {
                Location* tmp;
                if (right->GetType() == Type::stringType)
                        tmp = codegen->GenBuiltInCall(StringEqual,
                                                      Left_Emit,
                                                      Right_Emit);
                else
                        tmp = codegen->GenBinaryOp("==", Left_Emit, Right_Emit);
                tmp = codegen->GenBinaryOp("-",
                                           tmp,
                                           codegen->GenLoadConstant(1));
                tmp = codegen->GenBinaryOp("*",
                                           tmp,
                                           codegen->GenLoadConstant(-1));
                return tmp;
...
Location* LogicalExpr::Emit(CodeGenerator* codegen)
{
        Location* Right_Emit = right->Emit(codegen);
        if (strcmp(op->str(), "!") == 0) {
                Right_Emit = codegen->GenBinaryOp
                        ("-", Right_Emit, codegen->GenLoadConstant(1));
                Right_Emit = codegen->GenBinaryOp
                        ("*", Right_Emit, codegen->GenLoadConstant(-1));
                return Right_Emit;
        }
