  ...
  _L31:
	...
	# _tmp243 = _tmp242 == _tmp239
	  lw $t6, -76($fp)	# fill _tmp242 to $t6 from $fp-76
	  seq $t9, $t6, $t3	
	  sw $t9, -80($fp)	# spill _tmp243 from $t9 to $fp-80
	# _tmp244 = _tmp240 || _tmp243
	  lw $t3, -80($fp)	# fill _tmp243 to $t3 from $fp-80
	  or $t6, $t5, $t3	
	# IfZ _tmp244 Goto _L33
	  sw $t1, -8($fp)	# spill elem from $t1 to $fp-8
	  sw $t2, -60($fp)	# spill _tmp238 from $t2 to $fp-60
	  sw $t6, -84($fp)	# spill _tmp244 from $t6 to $fp-84
	  beqz $t6, _L33	# branch if _tmp244 is zero 
