...
void Mips::UpdateRegisterInfo(Register& reg, bool dirtyBit, Location* dst)
{
        regs[reg].var = dst;
        regs[reg].isDirty = dirtyBit;
}
...
void Mips::SpillRegister(Location *dst, Register reg)
{
        ...
        UpdateRegisterInfo(reg, false, NULL);
}

/* Method: FindRegister
 * --------------------
 * Find the register for a given location Otherwise, give an unused register.
 */
Mips::Register Mips::FindRegister(Location *src)
{
        if (src != NULL) { 
                if (regs[t0].var == src)
                        return t0;
...
/* Method: SpillAllRegisters
 * -------------------------
 * Used to spill all registers t0-t9
 */
void Mips::SpillAllRegisters()
{
        // std::cout << "SpillAllRegisters" << std::endl;
        if (regs[t0].var != NULL)
                SpillRegister(regs[t0].var, t0);
        if (regs[t1].var != NULL)
                SpillRegister(regs[t1].var, t1);
        if (regs[t2].var != NULL)
                SpillRegister(regs[t2].var, t2);
        if (regs[t3].var != NULL)
                SpillRegister(regs[t3].var, t3);
        if (regs[t4].var != NULL)
                SpillRegister(regs[t4].var, t4);
        if (regs[t5].var != NULL)
                SpillRegister(regs[t5].var, t5); 
        if (regs[t6].var != NULL)
                SpillRegister(regs[t6].var, t6);
        if (regs[t7].var != NULL)
                SpillRegister(regs[t7].var, t7);
        if (regs[t8].var != NULL)
                SpillRegister(regs[t8].var, t8);
        if (regs[t9].var != NULL)
                SpillRegister(regs[t9].var, t9);
}

/* Method: SpillThreeRegisters
 * ---------------------------
 * Used to spill registers t0-t2
 */
void Mips::SpillThreeRegisters()
{
        // std::cout << "SpillThreeRegisters" << std::endl;
        if (regs[t0].var != NULL)
                SpillRegister(regs[t0].var, t0);
        if (regs[t1].var != NULL)
                SpillRegister(regs[t1].var, t1);
...
{
        // std::cout << "EmitLoadConstant with location" << dst << std::endl;
        Register r = FindRegister(dst);
        UpdateRegisterInfo(r, true, dst);
...
{
        // std::cout << "EmitLoadStringConstant with location " << dst << std::endl;
        static int strNum = 1;
        char label[16];
...
{
        // std::cout << "EmitCopy with locations src, dst" << src << ", " << dst  << std::endl;
        Register r = FindRegister(src);
        if (regs[r].var != src) {
                FillRegister(src, r);
                UpdateRegisterInfo(r, false, src);
        }
        else
                SpillRegister(src, r);
        UpdateRegisterInfo(r, true, dst);
...
{
        // std::cout << "EmitLoad" << std::endl;
        Register r1 = FindRegister(reference);
        if (regs[r1].var != reference) {
                FillRegister(reference, r1);
                UpdateRegisterInfo(r1, false, reference);
        }
        Register r2 = FindRegister(dst);
        Emit("lw %s, %d(%s) \t# load with offset", regs[r2].name,
             offset, regs[r1].name);
        UpdateRegisterInfo(r2, true, dst);
        SpillThreeRegisters();
}
...
{
        // std::cout << "EmitStore" << std::endl;
        Register r1 = FindRegister(value);
        if (regs[r1].var != value) {
                UpdateRegisterInfo(r1, false, value);
                FillRegister(value, r1);
        }
        Register r2 = FindRegister(reference);
        if (regs[r2].var != reference) {
                UpdateRegisterInfo(r2, true, reference);
...
{
        // std::cout << "EmitBinaryOp" << std::endl;
        Register r1 = FindRegister(op1);
        if (regs[r1].var != op1) {
                UpdateRegisterInfo(r1, false, op1);
                FillRegister(op1, r1);
        }
        Register r2 = FindRegister(op2);
        if (regs[r2].var != op2) {
                UpdateRegisterInfo(r2, false, op2);
                FillRegister(op2, r2);
        }
        Register r3 = FindRegister(NULL);
        Emit("%s %s, %s, %s\t", NameForTac(code), regs[r3].name,
             regs[r1].name, regs[r2].name);
        UpdateRegisterInfo(r3, true, dst);
        SpillAllRegisters();
}
...
{
        Register r = FindRegister(test);
        if (regs[r].var != test) {
                FillRegister(test, r);
                UpdateRegisterInfo(r, true, test);
        }
...
void Mips::EmitReturn(Location *returnVal)
{
        if (returnVal != NULL) 
        {
                Register r = FindRegister(returnVal);
