...
template <class Value> void Hashtable<Value>::Combine (Hashtable <Value> *h) {

    if (h == NULL)
        return;
    
    typename std::multimap<const char*, Value>::iterator t;
    std::multimap<const char*, Value, ltstr> map_tmp = h -> GetMap();
    for (t = map_tmp.begin(); t != map_tmp.end(); t++){
        this -> Enter (t -> first, t -> second);
    }
}
