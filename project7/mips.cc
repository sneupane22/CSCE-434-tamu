...
{
        bool dead = false;
        if (livenessGraph != NULL) {
                const std::set<const Location*> out = livenessGraph->data_out(currentInstruction);
...
/* Method: EnsureRegister
 * ----------------------
 * If location not already in a register, load it to the register and return the register
 * If location is already in a register, simply return the register
 */
Mips::Register Mips::EnsureRegister(Location* src, Register fallback, bool load)
{
        for (int ri= t0; ri <= t9; ++ri)
        {
                if (regs[ri].var == src)
                        return (Register)ri;
        }
        ...
        for (int ri= t0; ri <= t9; ++ri)
        {
                if (regs[ri].var == NULL || IsDead((Register)ri))
                {
                        if (load)
                                FillRegister(src, (Register)ri);
                        regs[ri].var= src;
                        regs[ri].isDirty= false;
                        return (Register)ri;
                }
        }
        if (regs[fallback].isDirty)
                SpillRegister(regs[fallback].var, fallback);
        if (load)
                FillRegister(src, fallback);
        regs[fallback].var= src;
        regs[fallback].isDirty= false;
        return fallback;
}
 ...
 * --------------------
 */
void Mips::SpillForCall()
{
        for (int ri= t0; ri <= t9; ++ri)
        {
                if (IsDead((Register) ri))
                        continue;
                if (regs[ri].isDirty )
                        SpillRegister(regs[ri].var, (Register)ri);
                regs[ri].var= NULL;
                regs[ri].isDirty= false;
        }
}
 ...
 * -------------------
 */
void Mips::SpillGlobal()
{
        for (int ri= t0; ri <= t9; ++ri)
        {
                if (IsDead((Register)(ri)))
                        continue;
                if (regs[ri].isDirty && regs[ri].var->GetSegment() == gpRelative)
                        SpillRegister(regs[ri].var, (Register)ri);
                regs[ri].var= NULL;
                regs[ri].isDirty= false;
        }
 ...
 * -------------------
 */
void Mips::MarkChanged(Register reg)
{
...
{
        Register r =  EnsureRegister(dst, rd, false);
...
{
        Register r1= EnsureRegister(src, rs, true);
        Register r2= EnsureRegister(dst, rd, false);
        Emit("move %s, %s\t\t# copy value to register for %s",
             regs[r2].name, regs[r1].name, dst->GetName());
        MarkChanged(r2);
}
...
{
        Register r1= EnsureRegister(reference, rs, true);
        Register r2= EnsureRegister(dst, rd, false);
...
{
        Register r1= EnsureRegister(value, rs, true);
        Register r2= EnsureRegister(reference, rd, true);
...
{
        Register r1= EnsureRegister(op1, rs, true);
        Register r2= EnsureRegister(op2, rt, true);
        Register r3= EnsureRegister(dst, rd, false);
...
{
        Register r= EnsureRegister(test, rs, true);
        SpillAll();
...
{ 
        ...
        Register r= EnsureRegister(arg, rs, true);
...
void Mips::EmitCallInstr(Location *result, const char *fn, bool isLabel)
{
        ...
        if (result != NULL) {
                Register r= EnsureRegister(result, rd, false);
...
{
        Register r= EnsureRegister(fn, rs, true);
...
{ 
        if (returnVal != NULL) 
        {
                Register r= EnsureRegister(returnVal, rd, true);
